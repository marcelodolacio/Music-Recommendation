...
export function buildChatGraph(
  ...
  preferencerService: PreferencesService,
  memoryService: MemoryService
) {
  const graph = new StateGraph(ChatStateAnnotation)
    .addNode('chat', createChatNode(llmClient, preferencerService))
    .addNode('savePreferences', createSavePreferencesNode(preferencerService))
    .addNode('summarize', createSummarizationNode(llmClient, preferencerService))
  ...
  return graph.compile({
    checkpointer: memoryService.checkPoint,
